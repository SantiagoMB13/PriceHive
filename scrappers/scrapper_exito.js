...
const getExproduct = async (productName, prodindex, pagenum, refPrice) => {
    ...
    if (finalFilteredItems.length > prodindex) {
        ...
        let description;
        try {
            await page.$eval('div[data-fs-description-text=true]', element => element.innerText.trim());
        } catch (error) {
        ...
        try {
            ...
            if (specifications.trim == '<ul></ul>')
                specifications = 'No se encontraron especificaciones'
            else if (specifications == '<ul><li>Referencia: SIN REF</li></ul>')
                specifications = 'No se encontraron especificaciones'
            else
